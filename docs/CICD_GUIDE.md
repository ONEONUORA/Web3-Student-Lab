...
We use **GitHub Actions** for our CI/CD workflows. The pipeline is triggered automatically on every `push` to the main branch and for every `pull_request` targeting the main branch.
...
- **Frontend**: Runs `npm run build` (Next.js build) to verify the application can be bundled successfully.
